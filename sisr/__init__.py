...
def parse_resolution(resolution_str):
    ...
    if not resolution_str:
        return None, None
    parts = (
        resolution_str.split("x")
        if "x" in resolution_str
        else resolution_str.split(":")
    )
    if len(parts) != 2:
        return None, None
    try:
    ...
    try:
        ...
        if crop_type:
            if crop_type == "instagram":
                ...
                filter_chain.append(
                    "pad=w=1080:h=1080:x=(ow-iw)/2:y=(oh-ih)/2:color=black"
                )
                ...
                filter_chain.append(
                    f"crop={target_width}:{target_height}:0:{crop_y_offset}"
                )
        ...
        if overlay_type == "date":
            ...
            filter_chain.append(
                f"drawtext=text='%{{metadata\\:date}}':fontcolor=white@0.8:fontsize={fontsize}:box=1:boxcolor=black@0.5:boxborderw=10:x=(w*0.9)-tw:y=(h*0.9)-th:fontfile=/System/Library/Fonts/Courier.dfont"
            )
        elif overlay_type == "frame":
            filter_chain.append(
                "drawtext=text='FRAME\\: %{frame_num}':fontcolor=white@0.8:fontsize=96:box=1:boxcolor=black@0.5:boxborderw=5:x=(w*0.05)+(w*0.9-tw)/2:y=(h*0.05)+th"
            )
        ...
        if quality == "gif":
            ...
            cmd.extend(
                [
                    "-vf",
                    f"{','.join(filter_chain)},split[s0][s1];[s0]palettegen=max_colors=256:stats_mode=full[p];[s1][p]paletteuse=dither=sierra2_4a:diff_mode=rectangle",
                    "-loop",
                    "1",  # Loop forever
                    "-f",
                    "gif",
            ...
            if quality == "prores":
                cmd.extend(
                    [
                        ...
                        "+faststart",
                        "-qscale:v",
                        "9",  # Quality setting for standard ProRes
                    ]
                )
                ...
                cmd.extend(
                    [
                        ...
                        "+faststart",
                        "-qscale:v",
                        "5",  # Higher quality setting for ProRes HQ
                    ]
                )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create a video from images with optional date overlay and cropping"
    )
    ...
    parser.add_argument(
        "--instagram-crop",
        action="store_true",
        help="Crop to Instagram story format (1080x1920)",
    )
    parser.add_argument(
        "--hd-crop",
        choices=["center", "keep-top", "keep-bottom"],
        help="Crop to HD format (1920x1080)",
    )
    parser.add_argument(
        "--uhd-crop",
        choices=["center", "keep-top", "keep-bottom"],
        help="Crop to UHD format (3840x2160)",
    )
    parser.add_argument(
        "--overlay-date", action="store_true", help="Add date overlay to each frame"
    )
    parser.add_argument(
        "--overlay-frame",
        action="store_true",
        help="Add frame number overlay to each frame",
    )
    parser.add_argument(
        "--quality",
